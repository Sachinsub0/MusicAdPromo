...
@app.get("/health")
def health():return {"status":"ok","version":"MusicAdPromo-v6"}
@app.post("/analyze")
async def analyze(audio:UploadFile=File(...),lyrics:str=Form(""),duration:int=Form(12),selected_start:float|None=Form(None)):
 raw=await audio.read();p=save(audio,raw);c=rank_hooks(p,duration);start=float(selected_start if selected_start is not None else c[0]["start"]);tl=timeline(p,start,duration);words=align_words(p,start,duration,lyrics);creative=understand(lyrics,tl);words=decorate(words,tl)
 return {"clip_start":start,"duration":duration,"candidates":c,"timeline":tl,"creative":creative,"words":words,"image_prompt":image_prompt(creative)}
@app.post("/render")
